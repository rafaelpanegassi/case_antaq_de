...
if __name__ == "__main__":
    ...
    silver_table_path = f"s3a://{silver_bucket}/carga/"
    gold_table_path   = f"s3a://{gold_bucket}/carga/"

    logger.info(f"Reading data from Silver at: {silver_table_path}")
    ...
    if not table_exists:
        ...
        upsert_condition = (
            "t.destino = s.destino and t.cdmercadoria = s.cdmercadoria and "
            "t.origem = s.origem and t.idcarga = s.idcarga and t.idatracacao = s.idatracacao"
        )
        (
            delta_table.alias("t")
            .merge(
