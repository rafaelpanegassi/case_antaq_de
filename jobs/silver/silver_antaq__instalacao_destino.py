...
def normalize_col_name(col_name: str) -> str:
    ...
    col_name = col_name.replace(".", "_")   # Replace dots to avoid ambiguous column references
    return col_name
...
if __name__ == "__main__":
    ...
    spark = (
        ...
        .appName("SilverInstalacaoDestinoDelta")
        .master("local[*]")
    ...
    bronze_path = f"s3a://{bronze_bucket}/instalacao/instalacao_destino/*-{execution_date}.txt"
    ...
    delta_output_path = f"s3a://{silver_bucket}/instalacao_destino/"
    (
        df.write
        .format("delta")
        .mode("overwrite")
